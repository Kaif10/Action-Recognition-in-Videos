...
if __name__ == "__main__":
    ...
    model = ConvLSTM( num_classes=10, latent_dim=opt.latent_dim)
    model.to(device)
    #model.load_state_dict(torch.load(opt.checkpoint_model))
    #model.load_state_dict(torch.load(opt.checkpoint_model, map_location=torch.device('cpu')))
    model.load_state_dict(torch.load(opt.checkpoint_model, map_location=torch.device('cpu')), strict=False
)
